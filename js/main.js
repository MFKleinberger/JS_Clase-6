...
function solicitarCantidadProductos() {
    let cantidadProductos;
    do {
        cantidadProductos = parseInt(prompt("Por favor ingrese la cantidad de productos que desea crear (de 1 a 3)"));
        if (cantidadProductos < 1 || cantidadProductos > 3 || isNaN(cantidadProductos)) {
            alert("La cantidad está fuera de parámetro. Por favor, ingrese un número entre 1 y 3.");
        }
    } while (cantidadProductos < 1 || cantidadProductos > 3 || isNaN(cantidadProductos));
    return cantidadProductos;
}

const cantidadProductos = solicitarCantidadProductos();

for (let i = 0; i < cantidadProductos; i++) {
    const producto = new Producto ({nombre: prompt("Ingrese el nombre del Producto " + (i + 1) + ":"), precio: parseInt(prompt("Ingrese el precio del Producto " + (i + 1) + ":"))});
